...
from trainer_rerank import Trainer, TrainerCallback, LSTMSelector
...
def gen_c(tokenizer, passage, span, max_len, bound_tokens, d_start, d_end, no_additional_marker, mask_entity):
    ...
    for idx, (st, en) in enumerate(intervals):
        ...
        tmp_sentence = tokenizer.convert_tokens_to_string(tmp_tokens)
        response = ollama.embeddings(
            prompt=tmp_sentence,
            model="mxbai-embed-large"
        )["embedding"]
        lst_emb.append(response)


    return ret, intervals, target_interval, lst_emb
...
#
#     try:
#         senemb1 = json.load(open(f'{senemb_path}/{mod}/codred-doc-senemb-doc1-{h}-{t}-{doc1_id}-{doc2_id}',"r",encoding="utf-8"))
#         senemb2 = json.load(open(f'{senemb_path}/{mod}/codred-doc-senemb-doc2-{h}-{t}-{doc1_id}-{doc2_id}',"r",encoding="utf-8"))
#         relation_info = json.load(open(f'{senemb_path}/{mod}/codred-doc-rela-info-{h}-{t}-{doc1_id}-{doc2_id}',"r",encoding="utf-8"))
#         k1, intervals1, target_inte1, lst_emb1 = senemb1["tokens"], senemb1["intervals"], senemb1["target_inte"], senemb1["lst_emb"]
#         k2, intervals2, target_inte2, lst_emb2 = senemb2["tokens"], senemb2["intervals"], senemb2["target_inte"], senemb2["lst_emb"]
#     except FileNotFoundError as e:
#         k1, intervals1, target_inte1, lst_emb1 = gen_c(tokenizer, doc1['tokens'], v_h['spans'][0], max_len // 2 - 2, ['[unused1]', '[unused2]'], d1_start, d1_end, no_additional_marker, mask_entity)
#         k2, intervals2, target_inte2, lst_emb2 = gen_c(tokenizer, doc2['tokens'], v_t['spans'][0], max_len // 2 - 1, ['[unused3]', '[unused4]'], d2_start, d2_end, no_additional_marker, mask_entity)
#
#         relation_sentence = f'What is the relationship between {v_h["name"]} and {v_t["name"]} ?'
#         relation_emb = ollama.embeddings(
#                 prompt=relation_sentence,
#                 model="mxbai-embed-large"
#             )["embedding"]
#
#         senemb1 = {
...
#         json.dump(relation_info, open(f'{senemb_path}/{mod}/codred-doc-rela-info-{h}-{t}-{doc1_id}-{doc2_id}', 'w'))
#
#     return k1, intervals1, target_inte1, lst_emb1, k2, intervals2, target_inte2, lst_emb2

def cosine_similarity(a, b):
    return torch.nn.functional.cosine_similarity(a.unsqueeze(0), b.unsqueeze(0))

rerank_model_name = "/data/xywang/bge-reranker-large"
...
def process_example(args, h, t, doc1, doc2, tokenizer, path, answer,summary):
    senemb_path = args.senemb_path
    ...
    with open(f'{senemb_path}/{mod}/codred-doc-senemb-doc1-{h}-{t}-{doc1_id}-{doc2_id}', "r") as json_file:
        doc1 = json.load(json_file)
    with open(f'{senemb_path}/{mod}/codred-doc-senemb-doc2-{h}-{t}-{doc1_id}-{doc2_id}', "r") as json_file:
        doc2 = json.load(json_file)
    with open(f'{senemb_path}/{mod}/codred-doc-rela-info-{h}-{t}-{doc1_id}-{doc2_id}', "r") as json_file:
        rela_info = json.load(json_file)

    k1, intervals1, target_inte1 = doc1['tokens'], doc1['intervals'], doc1['target_inte']
    k2, intervals2, target_inte2 = doc2['tokens'], doc2['intervals'], doc2['target_inte']
    head_name, tail_name = rela_info['head'], rela_info['tail']
    ...
    for idx, (st, en) in enumerate(intervals1):
        all_k1_sentences.append(k1[st:en])
        all_k1_intervals.append((st,en))
    for idx, (st, en) in enumerate(intervals2):
        all_k2_sentences.append(k2[st:en])
        all_k2_intervals.append((st, en))


    k1_decoded_sentences = [tokenizer.convert_tokens_to_string(sen) for sen in all_k1_sentences]
    k2_decoded_sentences = [tokenizer.convert_tokens_to_string(sen) for sen in all_k2_sentences]

    query = f"What is the relationship between {head_name} and {tail_name} ?"

    temp_k1_reranked_intervals, _ = rerank_sentences(query, k1_decoded_sentences, all_k1_intervals)
    temp_k2_reranked_intervals, _ = rerank_sentences(query, k2_decoded_sentences, all_k2_intervals)
    k1_sorted_intervals = temp_k1_reranked_intervals[:15]
    k2_sorted_intervals = temp_k2_reranked_intervals[:15]

    k1_sorted_indices = [all_k1_intervals.index(inte) for inte in k1_sorted_intervals]
    k1_sorted_sentences = [all_k1_sentences[i] for i in k1_sorted_indices]
    k1_decoded_sentences = [tokenizer.convert_tokens_to_string(sen) for sen in k1_sorted_sentences]

    k2_sorted_indices = [all_k2_intervals.index(inte) for inte in k2_sorted_intervals]
    k2_sorted_sentences = [all_k2_sentences[i] for i in k2_sorted_indices]
    k2_decoded_sentences = [tokenizer.convert_tokens_to_string(sen) for sen in k2_sorted_sentences]

    querys = []
    querys.append(f"What is the relationship between {head_name} and {tail_name} ?")
    querys.append(f"What is the relationship between {head_name} and {tail_name} ? {answer}")
    if path == "n/a":
        querys.append(f"What is the relationship between {head_name} and {tail_name} ?")
        querys.append(f"What is the relationship between {head_name} and {tail_name} ? {answer}")
    else:
        querys.append(f"What is the relationship between {head_name} and {tail_name}, considering the entity path: {path} ?")
        querys.append(
            f"What is the relationship between {head_name} and {tail_name}, considering the entity path: {path} ? {answer}")
    querys.append(f"What is the relationship between {head_name} and {tail_name} ? {summary}")


    k1_rerank_scores = []
    k2_rerank_scores = []
    for query in querys:
        _, k1_rerank_score = rerank_sentences(query, k1_decoded_sentences, k1_sorted_intervals)
        _, k2_rerank_score = rerank_sentences(query, k2_decoded_sentences, k2_sorted_intervals)
        k1_rerank_scores.append(k1_rerank_score.cpu().tolist())
        k2_rerank_scores.append(k2_rerank_score.cpu().tolist())

    head_sentence = k1[target_inte1[0]:target_inte1[1]]
    tail_sentence = k2[target_inte2[0]:target_inte2[1]]
    if '[unused1]' not in head_sentence or '[unused2]' not in head_sentence:
        additional_sentence = tokenizer.tokenize("[unused1] " + head_name + " [unused2]")
        print(additional_sentence)
        if '[unused1]' not in additional_sentence or '[unused2]' not in additional_sentence:
            exit()
        head_sentence = additional_sentence + head_sentence
    if '[unused3]' not in tail_sentence or '[unused4]' not in tail_sentence:
        additional_sentence = tokenizer.tokenize("[unused3] " + tail_name + " [unused4]")
        print(additional_sentence)
        if '[unused3]' not in additional_sentence or '[unused4]' not in additional_sentence:
            exit()
        tail_sentence = tail_sentence + additional_sentence
    ...
    rerank1 = {
        ...
        'selected_intervals': k1_sorted_intervals,
        'rerank_score': k1_rerank_scores
    }
    ...
    rerank2 = {
        ...
        'intervals': intervals2,
        'target_inte': target_inte2,
        'sentence': tail_sentence,
        'selected_intervals': k2_sorted_intervals,
        'rerank_score': k2_rerank_scores
    }

    os.makedirs(f'{rerank_path}/{mod}', exist_ok=True)

    json.dump(rerank1, open(f'{rerank_path}/{mod}/codred-doc-rerank-doc1-{h}-{t}-{doc1_id}-{doc2_id}', 'w'))
    json.dump(rerank2, open(f'{rerank_path}/{mod}/codred-doc-rerank-doc2-{h}-{t}-{doc1_id}-{doc2_id}', 'w'))

    return k1, intervals1, target_inte1, k2, intervals2, target_inte2
...
def collate_fn(lst_batch, args, relation2id, tokenizer, redisd, sent_model):
    ...
    for i in range(len(lst_batch)):
        ...
        if batch[-1] == 'o':
            ...
            target_inte_codre = list()
            lst_emb_codre = list()
            dplabel = list()
            for doc1, doc2, l, path, answer,summary in dps:
                k1, intervals1, target_inte1, k2, intervals2, target_inte2 = process_example(args, h, t, doc1, doc2, tokenizer, path, answer,summary)
                tokens_codre.append((k1, k2))
                intervals_codre.append((intervals1, intervals2))
                target_inte_codre.append((target_inte1, target_inte2))
            ...
            lst_target_inte_codre.append(target_inte_codre)
    ...
    return lst_input_ids_t, lst_token_type_ids_t, lst_attention_mask_t, lst_dplabel_t, lst_rs_t, lst_r,\
           lst_tokens_codre, lst_intervals_codre, lst_target_inte_codre, lst_lst_emb_codre, lst_dp_label_codre, lst_rs_codre, lst_r_codre
...
def collate_fn_infer(batch, args, relation2id, tokenizer, redisd, sent_model):
    ...
    return tokens_codre, intervals_codre, target_inte_codre, lst_emb_codre, h, rs, t
...
def read_knowledge_file(dataset, graph_file, answer_file, summary_file):
    data_bags = []
    graph_samples = {}
    answer_samples = {}
    summary_samples = {}
    ...
    with(open(answer_file, "r", encoding="utf-8")) as file:
        for line in file:
            try:
                ...
                for doc1, doc2, l, answer in answer_sample[2]:
    ...
    with(open(summary_file, "r", encoding="utf-8")) as file:
        for line in file:
            try:
                summary_sample = json.loads(line.strip())
                for doc1, doc2, l, summary in summary_sample[2]:
                    summary_samples[summary_sample[0] + doc1 + doc2] = summary
    ...
    for sample in tqdm(dataset, total=len(dataset), desc="Processing"):
        if sample[-1] == "o":
            ...
            for doc1, doc2, l in sample[2]:
                ...
                new_dp.append(summary_samples[sample[0] + doc1 + doc2])
                dps.append(new_dp)
...
class CodredCallback(TrainerCallback):
    ...
    def on_argument(self, parser):
        ...
        parser.add_argument('--dev_file', type=str,
                            default='../../data/rawdata/dev_dataset.json')
        parser.add_argument('--test_file', type=str,
                            default='../../data/rawdata/test_dataset.json')
        parser.add_argument('--dsre_file', type=str, default='../../data/dsre_train_examples.json')
        parser.add_argument('--train_graph_file', type=str, default='../../data/path/train_path.json')
        parser.add_argument('--dev_graph_file', type=str, default='../../data/path/dev_path.json')
        parser.add_argument('--test_graph_file', type=str, default='../../data/path/test_path.json')
        parser.add_argument('--train_answer_file', type=str, default='../../data/answer/train_answer.json')
        parser.add_argument('--dev_answer_file', type=str, default='../../data/answer/dev_answer.json')
        parser.add_argument('--test_answer_file', type=str, default='../../data/answer/test_answer.json')
        parser.add_argument('--train_summary_file', type=str, default='../../data/summary/train_summary.json')
        parser.add_argument('--dev_summary_file', type=str, default='../../data/summary/dev_summary.json')
        parser.add_argument('--test_summary_file', type=str, default='../../data/summary/test_summary.json')
    ...
    def load_data(self):
        ...
        test_bags = place_dev_data(test_dataset, self.args.single_path)
        ...
        # train_bags = read_knowledge_file(train_bags, self.args.train_graph_file, self.args.train_answer_file, self.args.train_summary_file)
        dev_bags = read_knowledge_file(dev_bags, self.args.dev_graph_file, self.args.dev_answer_file, self.args.dev_summary_file)
        test_bags = read_knowledge_file(test_bags, self.args.test_graph_file, self.args.test_answer_file, self.args.test_summary_file)
